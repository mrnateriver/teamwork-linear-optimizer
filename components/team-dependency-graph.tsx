...
export function TeamDependencyGraph({ teamId }: TeamDependencyGraphProps) {
  ...
  useEffect(() => {
    ...
    const addNode = (
      project: ProjectType,
      team: Team | undefined,
      type?: "team" | "dependency" | "dependent",
    ) => {
    ...
    teamProjects.forEach((project) => {
      ...
      directDependencies.forEach((depId) => {
        ...
        if (depProject && depProject.teamId !== project.teamId) {
      ...
      directDependents.forEach((dependentId) => {
        ...
        if (dependentProject) {
          const dependentTeam = teams.find(
            (t) => t.id === dependentProject.teamId,
          );
    ...
    columnNodes.dependents.forEach((node, rowIndex) => {
      node.position = {
        x: initialX + 2 * xSpacing,
        y: 50 + rowIndex * ySpacing,
      };
    });
  ...
  if (teamProjects.length === 0) {
    return (
      <div className="p-4 text-center text-muted-foreground">
        No projects in this team to display dependencies for.
      </div>
    );
  }
